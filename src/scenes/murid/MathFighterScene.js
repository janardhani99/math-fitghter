...
export default class MathFighterScene extends Phaser.Scene
{
    ...
    preload()
    {
        ...
        this.load.image('start-btn','image/start_button.png')
        ...
        this.load.image('start-btn', 'image/start_button.png')
        this.load.image('newStart','image/new-start.png')
    }
    create()
    {
        ...
        this.enemy=this.physics.add.sprite(
            ...
            let start_button=this.add.image(this.gameHalfWidth,this.gameHalfHeight + 181,'newStart').setInteractive()
            start_button.on('pointerdown',()=>{this.gameStart();start_button.destroy()},this)
            this.physics.add.overlap(this.enemy,this.slash,this.spriteHit, null, this)
            this.physics.add.overlap(this.player,this.slash,this.spriteHit, null, this)
    }
    ...
    update(time)
    {
        if(this.correctAnswer == true && !this.playerAttack)
        {
            ...
            this.time.delayedCall(500, ()=>{
                this.createSlash(this.player.x + 60, this.player.y, 1, 600)
            })
    ...
    //membuat animasi dari player dan enemy
    createAnimation()
    {
        this.anims.create({
            key:'player-die',
